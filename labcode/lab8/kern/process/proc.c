...
load_icode_read(int fd, void *buf, size_t len, off_t offset)
{
    ...
    while (copied_total < len) {
        size_t copied = 0;
        ret = file_read(fd, (char *)buf + copied_total, len - copied_total, &copied);
        if (ret != 0) {
            return ret;
        }
        if (copied == 0) {
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (uint32_t i = 0; i < elf.e_phnum; i++)
    {
        ...
        uintptr_t file_start = ph.p_va;
        uintptr_t file_end = ph.p_va + ph.p_filesz;
        uintptr_t la = ROUNDDOWN(file_start, PGSIZE);
        ...
        // Zero bss part
        uintptr_t mem_start = ph.p_va + ph.p_filesz;
        uintptr_t mem_end = ph.p_va + ph.p_memsz;
        if (mem_start < mem_end)
        {
            la = ROUNDDOWN(mem_start, PGSIZE);
            while (mem_start < mem_end)
            {
                ...
                size_t off = mem_start - la;
                size_t size = PGSIZE - off;
                if (mem_end < la + PGSIZE)
                {
                    size -= (la + PGSIZE) - mem_end;
                }
    ...
    // Push argv strings and argv[] onto user stack
    ...
    // RISC-V ABI requires the stack to be 16-byte aligned at call boundaries.
    stacktop = ROUNDDOWN(stacktop, 16);
    size_t argv_bytes = (argc + 1) * sizeof(uintptr_t);
    if ((argv_bytes & 0xF) != 0)
    {
        stacktop -= 8;
    }
    stacktop -= argv_bytes;
    uintptr_t uargv_ptr = stacktop;

    uintptr_t zero = 0;
    for (int i = 0; i < argc; i++)
    {
        if (!copy_to_user(mm, (void *)(uargv_ptr + i * sizeof(uintptr_t)), &uargv[i], sizeof(uintptr_t)))
        {
            ret = -E_INVAL;
            goto bad_cleanup_mmap;
        }
    }
    if (!copy_to_user(mm, (void *)(uargv_ptr + argc * sizeof(uintptr_t)), &zero, sizeof(uintptr_t)))
    {
        ret = -E_INVAL;
        goto bad_cleanup_mmap;
    }

    // Setup trapframe for user env
